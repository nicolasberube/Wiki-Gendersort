...
CE CODE DOIT ÊTRE LANCÉ VIA LE SCRIPT BASH VIA LA COMMANDE SUIVANTE:
    
    /Users/berube/Desktop/postdoc/AssignationGenre/Assignation-reboot.sh NameKey.txt 25

suivi de la part2 pour la fusion des fichiers log:

    python /Users/berube/Desktop/postdoc/AssignationGenre/Assignation-reboot_part2.py NameKey.txt

Ce code fait l'équivalent de relancer le code Assignation-update.py pour toute la base de données sans considération pour la base de donnée existante. C'est un code très demandant et devrait être lancé pendant la nuit.
La ligne de commande bash permet de lancer le code sur plusieurs processus (via la commande screen) à la fois pour accélérer le temps de calcul (dans l'exemple donné, le chiffre 25 peut être changé pour le nombre de processus parallèles)
La complétion des calculs doit être vérifiée via la commande 'screen -ls'. Si des screens sont encore actifs, il faut attendre ou terminer les processus en cours.


Ce code prend une liste de prénoms séparés par un retour à la ligne \n dans un fichier .txt (NameKey.txt) et reconstruit une base de donnée (NameKeyOut.txt) en leur attribuant un genre.
L'attribution du genre (M, F, UNI, INI ou UNK) se fait selon l'occurence de prenoms sur des pages Wikipedia les concernant.
Les noms déjà présents dans la base de données existante (NameKeyOut.txt) ne sont pas considérés et sont tous recalculés.
...
Le code Assignation-reboot_part2.py doit ensuite être utilisé pour fusionner tous les fichier NameKeyLogX.txt dans un nouveau fichier NameKeyLog.txt
...
otempsn = ['']
otemps = [['','UNK','\nname is empty\n = UNK']]
if isfile(inputnames[:-4]+'Log.txt') and not reboot:
...
#ntrack: pour progress bar
ntrack = 0
filelog = open(inputnames[:-4]+'Log%i.txt'%cn,'w')
for name in names:
  ntrack += 1
